...
// The rules already sending this sender to Junk, whether they name the address outright or
// cover it with a substring or an expression. Returned so the undo path can name and delete them.
function junkRulesCovering({ accountId, folderId, email, name }) {
  return junkRules(accountId, folderId).filter((rule) => {
...
function queueMove(threads) {
  ...
  const movable = [...byId.values()].filter((t) => !isInSpam(t));
...
async function junkSenders(senders) {
  ...
  for (const sender of senders.filter((s) => s.rules.length === 0)) {
...
function isInSpam(thread) {
  return thread.folders.some((f) => f.role === 'spam');
}

// Undo: drop every rule covering the selected senders and put their mail back in the Inbox.
// A single pattern rule can cover several of the selected senders, so delete each rule once.
function unjunkSenders(senders) {
  const removed = [];
  for (const sender of senders) {
    for (const rule of sender.rules) {
      if (!removed.some((r) => r.id === rule.id)) {
        removed.push(rule);
  ...
  const threads = [].concat(...senders.map((s) => s.threads)).filter(isInSpam);
  if (threads.length > 0) {
    Actions.queueTasks(TaskFactory.tasksForMarkingNotSpam({ source: SOURCE, threads }));
    Actions.popSheet();
  }
  return { removed, moved: threads.length };
}
